...
mkdir build
cd build
arch --x86_64 cmake -D "CMAKE_INSTALL_PREFIX=$INSTALL_DIR" -D "BUILD_SHARED=OFF" -D "BUILD_TESTS=OFF" -D "BUILD_BENCHMARKS=OFF" ..
...
mkdir build
cd build
arch --x86_64 cmake -D "CMAKE_INSTALL_PREFIX=$INSTALL_DIR" ..
arch --x86_64 cmake --build . --config Release
...
mkdir build
cd build
arch --x86_64 cmake -D "CMAKE_INSTALL_PREFIX=$INSTALL_DIR" -D "BUILD_SHARED_LIBS=OFF" -D "PYILMBASE_ENABLE=0" -D "OPENEXR_VIEWERS_ENABLE=0" -D "ZLIB_INCLUDE_DIR=$AUTOREMESHER_BUILD_DIR/thirdparty/zlib/zlib-1.2.11" -D "ZLIB_LIBRARY=$AUTOREMESHER_BUILD_DIR/thirdparty/zlib/zlib-1.2.11/build/libz.a" ..
arch --x86_64 cmake --build . --config Release
cd $AUTOREMESHER_BUILD_DIR
